...
namespace Netjs
{
    ...
    public class TsOutputVisitor : IAstVisitor
    {
        ...
        void WriteIdentifier(string identifier, Role<Identifier> identifierRole = null)
        {
            ...
            formatter.WriteToken(identifier);
            lastWritten = LastWritten.KeywordOrIdentifier;
        }
        ...
        void WriteQualifiedIdentifier(IEnumerable<string> identifiers)
        {
            bool first = true;
        ...
        public void VisitTypeDeclaration(TypeDeclaration typeDeclaration)
        {
            ...
            if (typeDeclaration.BaseTypes.Any())
            {
                ...
                var bs = typeDeclaration.BaseTypes.FirstOrDefault(x => !IsInterface(x));
                ...
                var ints = typeDeclaration.BaseTypes.Where(IsInterface).ToList();
        ...
        public static bool IsInterface(Mono.Cecil.TypeDefinition td)
        {
            if (td == null)
                return false;
        ...
        static Mono.Cecil.TypeDefinition GetTypeDef(AstType type)
        {
            var td = type.Annotation<Mono.Cecil.TypeDefinition>();

            if (td == null)
            {
                var tr = type.Annotation<Mono.Cecil.TypeReference>();
                if (tr != null)
                {
                    td = tr.Resolve();
                }
        ...
        public void OpenBraceInternal(BraceStyle style)
        {
            switch (style)
            {
                case BraceStyle.DoNotChange:
        ...
        private TokenWriter writer => formatter;
        private bool isAtStartOfLine = true;
    }
    static class TypeScriptExtensionMethods
    {
        public static void WriteToken(this TokenWriter writer, string token)
        {
            writer.WriteToken(null, token);
        }
    ...
    public class OptionalParameterNote { }
